...
# این تابع رو ساختم که پیام تکراری ننویسیم
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if member.status in ["member", "administrator", "creator"]:
            ...
            else: await update.message.reply_text("لینک منقضی شده یا وجود ندارد. 😔")
            return
    except: pass
...
async def check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if member.status in ["member", "administrator", "creator"]:
            ...
            if key in links: await send_movie_link(query, key)
